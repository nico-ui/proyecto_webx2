<%-- 
    Document   : Enfermeros
    Created on : 25 ene 2021, 1:40:28
...
<%@page errorPage="error.jsp?de=enfermeros.jsp"%>
            ...
            <div class="table-responsive">
                <div class="body3">
                    <table class="table">
                        ...
                        <tbody> 
                            <c:forEach var="enfermeros" items="${listaDeEnfermeros}">
                                <c:set var = "tipo" scope = "session" value = "${enfermeros.entidad.tipoUsuario}"/>
                                <c:if test = "${tipo eq 'E'}">
                                    <tr>
                                        <td>
                                            <a class="btn btn-primary btn-xs"
                                               href="PersonalMedicoServlet?accion=ver&email=<c:out value="${enfermeros.entidad.email}" />">
                                                <c:out value="${enfermeros.entidad.cedula}" />
                                            </a>
                                        </td>
                                        <td><c:out value="${enfermeros.entidad.nombre}" /></td>
                                        <td><c:out value="${enfermeros.entidad.paterno}" /></td>
                                        <td><c:out value="${enfermeros.entidad.materno}" /></td>
                                        <td><c:out value="${enfermeros.entidad.especialidad}" /></td>
                                        <td><c:out value="${enfermeros.entidad.cargo}" /></td>
                                        <td>
                                            <a class="btn btn-success btn-xs"
                                               href="PersonalMedicoServlet?accion=nuevaActualizacion&email=<c:out value="${enfermeros.entidad.email}" />">
                                                Modificar
                                            </a>
                                            <a class="btn btn-danger btn-xs"
                                               href="PersonalMedicoServlet?accion=eliminar&email=<c:out value="${enfermeros.entidad.email}" />">
                                                Eliminar
                                            </a>
                                        </td>
                                    </tr>
